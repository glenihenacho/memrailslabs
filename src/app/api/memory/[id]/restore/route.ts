...
import { z } from 'zod';
import { restore, getRecord } from '@/lib/memory';
...
/** §4.4 — dispute is reversible doubt; the reversal is a first-class transition. */
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const result = restore(params.id, { ...parsed.data, changed_by: 'api' });
    await flushAuthority();
    return NextResponse.json(result);
  } catch (err) {
    const message = (err as Error).message;
    const status =
      message === 'memory_not_found' ? 404 : message === 'only_disputed_memory_can_be_restored' ? 409 : 500;
